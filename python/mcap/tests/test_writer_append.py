...
def test_append_mode():
    ...
    try:
        ...
        metadatas = [r for r in appended_records if isinstance(r, Metadata)]
        assert len(metadatas) == 1
        assert metadatas[0].name == "metadata1"
        assert metadatas[0].metadata == {"test": "testValue"}
...
def test_append_conflict():
    ...
    try:
        ...
        assert "differs from previous channel record" in str(excinfo.value)
